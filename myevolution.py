...
class MySection:
    ...
    def initRand(self):
        for i in range(self.totalbeat):
            while True:
                temp = random.randint(1, 47)
                if ((temp >= 1 and temp <= 7) or (temp >= 11 and temp <= 17) or (temp >= 21 and temp <= 27) or
                        (temp >= 31 and temp <= 37) or (temp >= 41 and temp <= 47)):
                    break
...
class MyScore:
    ...
    def saveMyScore(self, savepath='./temp/', filename='bestScore',singer=0):
        ...
        with open(savepath+filename+'fitness.txt','w') as f:
    ...
    def evaluatesong(self, mymodle):
        ...
        # 转化歌曲
        self.saveMyScore()
        offset = 0.0010076305654536947
        time = 60
        sampletime = 0.25
        notenum = getnoteindex('./temp/bestScore.mid', time, offset, sampletime)
        if len(notenum) != 0:
            notenum = np.array(notenum[0:int(time / sampletime)])
            notenumreshape = notenum.reshape(16, int(time / sampletime / 16))
        # notenum = np.array(notenum)
        notenumreshape = notenumreshape.reshape(1,notenumreshape.shape[0], notenumreshape.shape[1])
        # 预测符合概率
        predic = mymodle.predict(notenumreshape, verbose=0)
        return predic[0][0]
...
class Environment:
    def __init__(self):
        ...
        self.totalbeat = 0
        self.model = None
        self.fullScore = [MyScore() for i in range(100)]
        self.style = 0
        self.createmode = 0
    ...
    def readcomponent(self):
        # 读取器件库
        import os
        folder_path = "./trainset/type/pianduan"  # 替换为您要读取的文件夹路径
        txt_files = [file for file in os.listdir(folder_path) if file.endswith(".txt")]
        num_txt_files = len(txt_files)
        for n in range(num_txt_files):
            rad = "./trainset/type/pianduan/" + str(n).zfill(5) + "pianduan.txt"
    ...
    # 初始化
    def musicinit(self, populationSize = 100, totalbeat = 16, style=1):
        # 读取歌曲库
        self.readcomponent()
        self.style = style - 1
        self.createmode = 0
                ...
                for serchchangenum in range(7):
                    ...
                    if change[0]:
                        ...
                        if serchindexresult[0] != -1:
                            ...
                            notelisttemp = midifynoteslist[:]
                            for i in range(sum(serchindexresult[1])):
                                midifynoteslist[serchindexresult[0]+i]=notelisttemp[serchindexresult[0]+sum(serchindexresult[1])-i-1]
                            # 更新时长表
                            NowtBestMyScore.getMyScorenote(sectionindex)
                            # print(3)
                    elif change[4]:
        ...
        while True:
            ...
            if generation%2==0:
                for i in range(len(self.population)):
                    ...
                    # 进行适应度计算
                    i_MyScore.fitness = i_MyScore.evaluatesong(self.model)
                    i_MyScore.saveMyScore(savepath='./temp/music/generate/', filename=str(i + 20 * savecount + 1))
                savecount+=1
            # 繁衍代数大于generationCount退出, 并输出最佳解
            if generation >= generationCount or notevolvenum == childrennum:
                ...
                for i in range(len(self.population)):
                    ...
                    # 进行适应度计算
                    i_MyScore.fitness = i_MyScore.evaluatesong(self.model)
                    fitnesslist.append(i_MyScore.fitness)
    ...
    # 风格确定
    def musicstyle(self, songstyle=['A', 'B', 'C'],evonum=0):
        songstylecount = [len(list(v)) for k, v in groupby(songstyle)]
        print('开始生成')
        # for i in range(evonum):
        # 主歌部分
        for i in range(len(songstylecount)):
            self.musicinit(populationSize=100)
            ResultBestMyScore = self.run(6)
            # 尾部延长
            # 保存第一段
            for j in range(len(ResultBestMyScore)):
                self.fullScore[j].spliceexisscore(self.longmodify(self.replicate(ResultBestMyScore[j])))
            if songstylecount[i] > 1:
                for j in range(songstylecount[i]-1):
                    # 修改后尾
                    for k in range(len(ResultBestMyScore)):
                        self.fullScore[k].spliceexisscore(self.longmodify(self.copymodify(
                            self.replicate(ResultBestMyScore[k]))))

        # for i in range(20):
        #     self.fullScore[i].fitness = self.fullScore[i].evaluatesong(self.model)
        #     self.fullScore[i].saveMyScore(savepath='./temp/music/generate/', filename=str(i+20*evonum+1))
